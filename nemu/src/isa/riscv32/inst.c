...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(((int)src1 + imm)&(~0x03u), 1));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1+imm ; R(rd)= s->pc + 4) ;
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, if(src1 < imm){R(rd) = 1;}else{R(rd) = 0; } );  
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(((int)src1 + imm)&(~0x03u), 4));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(((int)src1 + imm)&(~0x03u), 2), 16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(((int)src1 + imm)&(~0x03u), 2));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(((int)src1 + imm)&(~0x03u), 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(((int)src1 + imm)&(~0x03u), 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(((int)src1 + imm)&(~0x03u), 2, src2));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", divi   , RE, R(rd) = (int32_t)src1 / (int32_t)src2);
