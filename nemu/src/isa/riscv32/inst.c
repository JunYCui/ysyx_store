...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , M, if(rd == 0){cpu.csr_reg[imm] = src1; } else {R(rd) = cpu.csr_reg[imm]; cpu.csr_reg[imm] = src1;} );
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , M, if(rd == 0){cpu.csr_reg[imm] = cpu.csr_reg[imm]|src1; } else {R(rd) = cpu.csr_reg[imm]; cpu.csr_reg[imm] = cpu.csr_reg[imm]|src1;} );
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = cpu.csr_reg[MEPC]); 
