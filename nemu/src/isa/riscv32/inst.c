...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (uint64_t)src1 >> BITS(imm,4,0)  );//立即数逻辑右移
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm );  
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm ); 
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << BITS(imm,4,0)  ); //立即数逻辑左移


  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, (uint64_t)src2));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, (uint64_t)src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, (uint64_t)src2));
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , RE, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", divi   , RE, R(rd) = src1 / src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , RE, R(rd) = (uint64_t)src1 / (uint64_t)src2);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , RE, R(rd) = ((uint64_t)(src1 * src2) >> 32));


  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0 q
