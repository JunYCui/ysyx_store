...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , M, if(rd == 0){csr_reg[imm] = src1; } else {rd = csr_reg[imm]; csr_reg[imm] = src1;} );
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrr  , M, if(rd == 0){csr_reg[imm] = csr_reg[imm]|src1; } else {rd = csr_reg[imm]; csr_reg[imm] = csr_reg[imm]|src1;} );
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(2,s->pc);); 
