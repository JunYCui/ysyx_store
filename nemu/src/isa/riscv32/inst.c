...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div1   , RE, if(src2!=0) {R(rd) = (int32_t)src1 / (int32_t)src2 ;} else {R(rd) = -1;} );
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , RE, if(src2!=0) {R(rd) = (uint64_t)src1 / (uint64_t)src2;} else {R(rd) = 0xffffffff;});
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , RE, if(src2 !=0){R(rd) = (int32_t)src1 % (int32_t)src2 ;} else {R(rd)=src1;});
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhu  , RE, R(rd) = (int64_t)(int32_t)src1 * (int64_t)(uint32_t)src2 >>32);
