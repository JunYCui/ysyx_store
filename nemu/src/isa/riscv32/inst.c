...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + (uint32_t)imm);// PC加立即数
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui  , U, R(rd) = (uint32_t)imm);// 立即数加载
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, SEXT(R(rd) = Mr(src1 + imm, 1), 8));
