...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr((src1 + imm)&(int)(~0x03u), 4));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr((src1 + imm)&(~0x03u), 2), 16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr((src1 + imm)&(int)(~0x03u), 2));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw((src1 + imm)&(int)(~0x03u), 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw((src1 + imm)&(int)(~0x03u), 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw((src1 + imm)&(int)(~0x03u), 2, src2));
